...
import { setCabTypeStatus } from "@/redux-toolkit/reducers/cab-filter";
...
const CabTypeFilter: React.FC = () => {
  ...
  const handleCheckboxChange = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (isChecked) {
      dispatch(setCabTypeStatus([...cabTypeStatus, value]));
    } else {
      dispatch(setCabTypeStatus(cabTypeStatus.filter((selectedValue: string) => selectedValue !== value)));
    }
  };

  useEffect(() => {
    dispatch(setCabTypeStatus(cabTypeStatus));
  }, [cabTypeStatus]);
