...
import { setFlightStatus } from "@/redux-toolkit/reducers/tour-filter";
...
const FlightFilter: React.FC = () => {
  ...
  const handleCheckboxChange = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (isChecked) {
      dispatch(setFlightStatus([...flightStatus, value]));
    } else {
      dispatch(setFlightStatus(flightStatus.filter((selectedValue: string) => selectedValue !== value)));
    }
  };

  useEffect(() => {
      dispatch(setFlightStatus(flightStatus));
  }, [flightStatus]);
