...
import { setTravelStatus } from "@/redux-toolkit/reducers/tour-filter";
...
const TravelFilter: React.FC = () => {
  ...
  const handleCheckboxChange = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (isChecked) {
      dispatch(setTravelStatus([...travelStatus, value]));
    } else {
      dispatch(setTravelStatus(travelStatus.filter((selectedValue: string) => selectedValue !== value)));
    }
  };

  useEffect(() => {
      dispatch(setTravelStatus(travelStatus));
  }, [travelStatus]);
