...
import { setPopularStatus } from "@/redux-toolkit/reducers/restaurant.filter";
...
const PopularFoodFilter: React.FC = () => {
  ...
  const handleCheckboxChange = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (isChecked) {
      dispatch(setPopularStatus([...popularStatus, value]));
    } else {
      dispatch(setPopularStatus(popularStatus.filter((selectedValue) => selectedValue !== value)));
    }
  };

  useEffect(() => {
      dispatch(setPopularStatus(popularStatus));
  }, [popularStatus]);
